...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $title ?? 'Hệ thống Đăng ký Thọ giới' }} - GHPGVN</title>
        ...
        <style>
            ...
            .logo-text-main span.red { color: #ef4444; }
            .logo-text-sub {
                font-size: 0.6rem;
                color: #fef3c7;
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center h-[72px] gap-6">
                    ...
                    <nav class="hidden md:flex items-center gap-1 flex-1">
                        <a href="{{ route('home') }}"
                           class="px-3 py-2 rounded-lg text-sm font-semibold transition-all {{ request()->is('/') ? 'bg-white/20 text-white' : 'text-amber-100 hover:bg-white/10 hover:text-white' }}">
                            Trang chủ
                        </a>
                        <a href="{{ route('application.track') }}"
                           class="px-3 py-2 rounded-lg text-sm font-semibold transition-all {{ request()->is('tra-cuu*') ? 'bg-white/20 text-white' : 'text-amber-100 hover:bg-white/10 hover:text-white' }}">
                            Tra cứu hồ sơ
                        </a>
                    </nav>
                    ...
                    <div class="ml-auto flex items-center gap-3 shrink-0">
                        @auth
                            {{-- User badge --}}
                            @if(in_array(Auth::user()->role, ['admin', 'quan_ly_tinh', 'quan_ly_gioi_dan']))
                            <a href="/admin"
                               class="hidden sm:inline-flex items-center gap-1.5 bg-white/15 hover:bg-white/25 text-white text-xs font-semibold px-3 py-2 rounded-lg transition-all border border-white/20">
                            ...
                            @endif
                            <a href="{{ route('profile') }}" class="hidden sm:flex items-center gap-2.5 bg-white/15 hover:bg-white/25 rounded-xl px-3 py-1.5 transition-all">
